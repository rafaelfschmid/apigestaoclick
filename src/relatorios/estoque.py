...
def valor_em_estoque():
    ...
    f.write(str(datetime.now()) + '\t' + str(valor_custo) +
            '\t' + str(valor_venda) + '\n')
    f.close()
...
def lista_bicicletas():
    ...
    for entry in entries:
        ...
        for item in json_obj['data']:
            produto = item['nome']
            # and (produto.find('SENSE') or produto.find('TSW') or produto.find('RAVA') or produto.find('NATHOR')) or (produto.find('BIC') and produto.find('TSW'))):
            if(isBike(produto)):
